...
func main() {
	...
	go func() {
		...
		mgsUrl := fmt.Sprintf("wss://ssmmessages.us-west-2.amazonaws.com/v1/control-channel/%s?role=subscribe&stream=input", instanceId)
		...
		go func() {
			for {
				...
				if err := agentMessage.Deserialize(rawMessage); err != nil {
					fmt.Printf("ReadMessage deser failure %v %v \n", messageType, rawMessage)
				}
				...
				case "agent_job":
					...
					if strings.HasPrefix(agentJobPayload.Topic, "aws.ssm.sendCommand") {
						docState, err := parseAgentJobSendCommandMessage(agentJobPayload, instanceId, *agentMessage)
						if err != nil {
							fmt.Printf("parseAgentJobSendCommandMessage error: %v\n", err)
							continue
						}
						fmt.Println(docState)
						// do something

						err = buildAgentJobAckMessageAndSend(agentMessage.MessageId, docState.DocumentInformation.MessageID, agentMessage.CreatedDate, Successful, conn)
						if err != nil {
						...
						sendDocResponse(payloadDoc, docState, conn)

						//docState.DocumentInformation.DocumentStatus = ResultStatusSuccess
						...
						docState.DocumentInformation.DocumentStatus = ResultStatusSuccess
						sendDocResponse(payloadDoc2, docState, conn)
					}
				}
			}
		}()
